...
public class UserActionsToRequestsApplier {
    ...
    public boolean isUserActionAffectingRequest(@NonNull UserActionEntity userAction) {
        return userAction.getEntityType() != null
                && userAction.getEntityType().equals(IDoCareContract.UserActions.ENTITY_TYPE_REQUEST);
    }
    ...
    public RequestEntity applyUserActionToRequest(@NonNull UserActionEntity userAction,
        ...
        if (!userAction.getEntityType().equals(IDoCareContract.UserActions.ENTITY_TYPE_REQUEST)
        ...
        if (userAction.getActionType().equals(IDoCareContract.UserActions.ACTION_TYPE_CLOSE_REQUEST)) {
            return closeRequest(CloseRequestUserActionEntity.fromUserAction(userAction), requestEntity);
        } else if (userAction.getActionType().equals(IDoCareContract.UserActions.ACTION_TYPE_PICKUP_REQUEST)) {
            return pickUpRequest(PickUpRequestUserActionEntity.fromUserAction(userAction), requestEntity);
        } else if (userAction.getActionType().equals(IDoCareContract.UserActions.ACTION_TYPE_CREATE_REQUEST)) {
            // this action is just a "marker" - the actual data has already been written to requests cache
            return requestEntity;
        } else if (userAction.getActionType().equals(IDoCareContract.UserActions.ACTION_TYPE_VOTE_FOR_REQUEST)) {
